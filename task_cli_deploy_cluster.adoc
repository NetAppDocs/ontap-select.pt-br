...
.RAID de hardware
--
Se você utiliza um controlador RAID local, é necessário criar pelo menos um datastore (ESXi) ou um link:kvm-host-configuration-and-preparation-checklist.html#step-2-create-a-storage-pool["pool de armazenamento (KVM)"] em cada nó para os dados do sistema, bem como para os agregados raiz e de dados. Você deve anexar o pool de storage como parte de configurar o nó ONTAP Select.

--
.RAID de software
--
Se você utiliza RAID por software, é necessário criar pelo menos um datastore (ESXi) ou um link:kvm-host-configuration-and-preparation-checklist.html#step-2-create-a-storage-pool["pool de armazenamento (KVM)"] para os dados do sistema e garantir que as unidades SSD estejam disponíveis para os agregados raiz e de dados. Você deve conectar o pool de storage e os discos como parte da configuração do nó ONTAP Select.
