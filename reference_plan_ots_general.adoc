...
Para obter mais informações sobre como preparar seu servidor Linux, incluindo a configuração e os pacotes de software necessários, consulte o link:kvm-host-configuration-and-preparation-checklist.html["lista de verificação de configuração do host"]. Consulte os requisitos do hipervisor para as distribuições Linux atualmente suportadas.
...
Consulte as informações de planejamento específicas do hipervisor e link:reference_release_notes.html["notas de lançamento"] para obter detalhes adicionais sobre as plataformas suportadas.
