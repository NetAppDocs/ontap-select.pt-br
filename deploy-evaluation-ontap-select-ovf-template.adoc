...
summary: Você pode implantar uma instância de avaliação de 90 dias de um cluster ONTAP Select de nó único. 
---
...
[role="lead"]
Você pode implantar uma instância de avaliação de 90 dias de um cluster ONTAP Select, com tamanho variando de um único nó até 99 nós, usando um modelo OVF no VMware ESXi. Você pode baixar uma licença de avaliação se já for cliente ou se for um novo cliente. Por exemplo, você já é cliente e deseja experimentar uma nova configuração antes de comprar uma licença.
...
== Etapa 1: Prepare o host do cluster ONTAP Select
...
== Etapa 2: Implante um cluster ONTAP Select de nó único usando um modelo OVF
...
** Selecione *Datacenter*. Em seguida, clique com o botão direito do mouse e selecione *Implantar modelo OVF*.
