...
NOTE: A extensão máxima e o tamanho do datastore para versões mais antigas do ESXi é de 64TB, o que pode afetar o número de LUNs necessários para suportar a capacidade bruta total fornecida por esses discos de grande capacidade.
...
NOTE: A partição NVRAM é separada em seu próprio VMDK. Esse VMDK é anexado usando o driver vNVME disponível nas versões do ESXi 8.0 ou posteriores. Essa mudança é mais significativa para instalações do ONTAP Select com RAID por software, que não se beneficiam do cache do controlador RAID.
