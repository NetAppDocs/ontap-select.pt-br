...
. Selecione a guia *Administração* na parte superior da página e, em seguida, selecione *Network Checker*.
. Selecione *Iniciar nova execução* e, em seguida, selecione os hosts e as redes para o par de HA
+
Você pode adicionar e configurar pares de HA adicionais conforme necessário.

. Selecione *Iniciar* para começar o teste de conectividade de rede.
