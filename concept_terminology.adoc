...
Para obter mais informações, consulte as duas seções link:concept_workflow_deploy.html["Plano"] e link:concept_lic_evaluation.html["Licença"].
...
Armazenamento de credenciais:: O repositório de credenciais de Deploy é um banco de dados seguro que armazena as credenciais da conta. Ele é usado principalmente para registrar hosts de hipervisor como parte da criação de um novo cluster. Consulte a seção link:concept_workflow_deploy.html["Plano"] para mais informações.
...
+
--
As configurações padrão do vNAS habilitam um recurso de otimização de gravação conhecido como registro de dados de instância única (SIDL). Com o ONTAP Select 9.6 e versões posteriores, os recursos de eficiência de storage em segundo plano do ONTAP são qualificados com o SIDL habilitado. Consulte a seção link:concept_stor_concepts_chars.html["Mergulho profundo"] para mais informações.
