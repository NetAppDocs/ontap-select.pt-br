...
. Selecione o ícone de figura no canto superior direito da página e, em seguida, selecione *Alterar senha*.
. Forneça a senha atual e a nova senha conforme solicitado e selecione *Enviar*.
...
. Selecione *Servidores de Gerenciamento* > *Adicionar vCenter*.
. Insira as seguintes informações e selecione *Adicionar*.
+
[cols="30,70"]
|===
...
|===
. Opcionalmente, selecione image:icon_kebab.gif["Opções"] após a adição do novo servidor de gerenciamento, e então selecione uma das seguintes opções:
+
