...
Existem dois modelos de implantação de alta disponibilidade (HA) do ONTAP Select : clusters com vários nós (quatro, seis, oito, dez ou doze nós) e clusters com dois nós. A principal característica de um cluster ONTAP Select de dois nós é o uso de um serviço de mediação externo para resolver cenários de dupla personalidade.  A máquina virtual ONTAP Deploy serve como mediadora padrão para todos os pares de alta disponibilidade (HA) de dois nós que ela configura.
