...
NOTE: A arquitetura de caixa de correio e o método de pulsação baseado em disco para resolver problemas de quorum e split-brain do cluster são os motivos pelos quais a variante multi-nó do ONTAP Select requer quatro nós separados ou um mediador para um cluster de dois nós.
