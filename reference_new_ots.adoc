...
Suporte para gerenciamento de chaves externas (KMIP):: A partir do ONTAP Select 9.19.1, o ONTAP Select oferece suporte ao gerenciamento de chaves externas usando o Key Management Interoperability Protocol (KMIP). Você pode habilitar o gerenciamento de chaves externas após implantar um cluster ONTAP Select 9.19.1 ou atualizar um cluster existente para o ONTAP Select 9.19.1. O gerenciamento de chaves externas é o único gerenciamento de chaves compatível com o ONTAP Select 9.19.1. Para mais informações, consulte link:reference_lic_ontap_features.html#licensed-ontap-features["Recursos do ONTAP licenciados por padrão"].
+
--
ONTAP Select não é compatível com gerenciadores de chaves na nuvem nem com o ONTAP Onboard Key Manager.

--
Suporte para processadores Advanced Micro Devices (AMD):: A partir da ONTAP Select 9.19.1, o ONTAP Select oferece suporte a processadores AMD em hosts hipervisor Kernel-Based Virtual Machine (KVM) e ESXi. Para mais informações, consulte link:reference_plan_ots_hardware.html["Considerações sobre o hipervisor e o hardware da VMware"].
...
Atualizações nas opções de download do produto ONTAP Select:: As opções de download do produto ONTAP Select no site de suporte da NetApp estão mudando com o lançamento do ONTAP Select 9.18.1 para *ONTAP Select Deploy* e *ONTAP Select Image*. Essas duas novas opções de download substituem as quatro opções anteriores, conforme mostrado na tabela a seguir:
+
--
[cols="30,30,40"]
|===
| Opção de download do produto anterior | Nova opção de download do produto | Para obter mais informações, acesse… 


| ONTAP Select | ONTAP Select Deploy | link:task_install_deploy.html["Instale o ONTAP Select Deploy"] 


| ONTAP Select Instalação de imagem | Imagem do ONTAP Select | link:task_cli_deploy_image_add.html["Adicione uma imagem ONTAP Select para implantar"] 


| Atualização do ONTAP Select Deploy | ONTAP Select Deploy | link:task_cli_upgrade_deploy.html["Atualizar uma instância do ONTAP Select Deploy"] 


| Atualização do nó ONTAP Select | Imagem do ONTAP Select | link:concept_adm_upgrading_nodes.html["Atualizar nós do ONTAP Select"] 
|===
--
Apoio reforçado para a expansão e contração de clusters:: A partir do ONTAP Select 9.18.1, a expansão e a contração de clusters são suportadas entre clusters ONTAP Select de quatro e doze nós em hosts de hipervisor ESXi e KVM.
+
--
--
Você pode aumentar o tamanho do cluster existente em um host ESXi ou KVM nos seguintes incrementos:
...
Você pode diminuir o tamanho do cluster existente em um host ESXi ou KVM nos seguintes incrementos:
...
--
Suporte ao hipervisor KVM atualizado:: A partir do ONTAP Select 9.18.1, o hipervisor KVM é suportado no Rocky Linux 10.1.
...
Suporte RAID de software para discos NVMe conectados localmente em hosts KVM:: ONTAP Select 9.17.1 introduz suporte a RAID por software para discos NVMe conectados localmente ao ONTAP Select em um host hipervisor KVM.
+
--
...
--
Suporte aprimorado para expansão e contração de cluster em hosts ESXi:: A partir do ONTAP Select 9.17.1, a expansão e a contração de clusters são suportadas entre clusters de seis e doze nós em hosts ESXi:
+
--
--
...
--
Suporte para SnapMirror cloud:: A partir do ONTAP Select 9.17.1, SnapMirror cloud é compatível com ONTAP Select. SnapMirror cloud é um recurso licenciado do ONTAP que é ativado por padrão ao implantar um cluster ONTAP Select 9.17.1 ou ao atualizar um cluster ONTAP Select existente para a versão 9.17.1.
+
--
https://docs.netapp.com/us-en/ontap/concepts/snapmirror-cloud-backups-object-store-concept.html["Saiba mais sobre SnapMirror cloud backups para storage de objetos"^].
...
+
--
O vSAN ESA é uma configuração específica do ESXi e uma nova arquitetura para VMware vSAN introduzida na vSphere 8. O vSAN ESA foi projetado para oferecer maior eficiência, escalabilidade e desempenho, principalmente ao usar dispositivos flash TLC baseados em NVMe.

ONTAP Select Deploy e ONTAP Select para ESXi suportam a configuração de um cluster de nó único e de vários nós do ONTAP Select, usando um datastore do tipo vSAN ou array externo para seu pool de storage.
...
--
Suporte ao hipervisor KVM atualizado:: A partir do ONTAP Select 9.17.1, o hipervisor KVM é suportado no Red Hat Enterprise Linux (RHEL) 10.1, 10.0, 9.7 e 9.6 e no Rocky Linux 10.0, 9.7 e 9.6.
+
--
[NOTE]
====
...
Suporte atualizado ao arquivo de licença do NetApp:: A partir do ONTAP Select 9.16.1, o suporte ao NetApp License File (NLF) foi atualizado. O novo formato NLF inclui licenças para os recursos ARP, ONTAP S3 e S3 SnapMirror. link:reference_lic_ontap_features.html#licensed-ontap-features["Saiba mais"].
+
--
...
--
Suporte para Autonomous ransomware Protection:: ONTAP Select 9.16.1 introduz suporte para Proteção Autônoma contra Ransomware (ARP). ONTAP Select 9.16.1 suporta apenas atualizações manuais para ARP, atualizações automáticas não são suportadas. A licença do recurso ARP está incluída no NLF para ONTAP Select 9.16.1. link:reference_lic_ontap_features.html#licensed-ontap-features["Saiba mais"].
...
NOTE: O suporte para expansão e contração de clusters é limitado a clusters ESXi somente.
...
Suporte ao ONTAP Select Deploy atualizado:: Se você estiver executando uma versão do ONTAP Select Deploy 9.14.1 inferior à 9.14.1P2, deverá atualizar para o ONTAP Select Deploy 9.14.1P2 o mais rápido possível. Para mais informações, consulte o link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["Notas de versão do ONTAP Select 9.14.1"^].
...
Suporte ao ONTAP Select Deploy atualizado:: A partir de abril de 2024, ONTAP Select Deploy 9.13.1 não está mais disponível no NetApp Support Site. Se você estiver executando ONTAP Select Deploy 9.13.1, deve atualizar para ONTAP Select Deploy 9.14.1P2 o mais rápido possível. Para mais informações, consulte o link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["Notas de versão do ONTAP Select 9.14.1"^].
...
A partir de abril de 2024, ONTAP Select Deploy 9.12.1 não está mais disponível no site de suporte da NetApp. Se você estiver executando ONTAP Select Deploy 9.12.1, deve atualizar para ONTAP Select Deploy 9.14.1P2 o mais rápido possível. Para mais informações, consulte o link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["Notas de versão do ONTAP Select 9.14.1"^].
...
+
NetApp recomenda enfaticamente que os clientes planejem e executem uma migração completa de dados do ONTAP Select para KVM para qualquer outra plataforma ONTAP, incluindo ONTAP Select para ESXi. Para mais informações, consulte o  https://mysupport.netapp.com/info/communications/ECMLP2877451.html["Aviso EOA"^]
