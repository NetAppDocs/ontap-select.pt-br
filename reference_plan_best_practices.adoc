...
A NetApp recomenda o uso do ESX 8.0 ou posterior e um disco NVMe para o armazenamento de dados que hospeda os discos do sistema. Essa configuração proporciona o melhor desempenho para a partição NVRAM .


NOTE: Ao instalar no ESX 8.0 ou posterior, o ONTAP Select usa o driver vNVMEe independentemente de o disco do sistema estar em um SSD ou em um disco NVMe. Isso define o nível de hardware da VM como 13, compatível com o ESX 8.0 e posteriores.
