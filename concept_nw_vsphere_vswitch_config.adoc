...
Você pode atribuir quatro grupos de portas a cada nó em um cluster com vários nós. Cada grupo de portas possui uma única porta física ativa e três portas físicas em espera, conforme ilustrado na figura a seguir.
...
O exemplo a seguir mostra a configuração de um vSwitch e os dois grupos de portas responsáveis ​​por lidar com os serviços de comunicação interna e externa para um cluster ONTAP Select de vários nós. A rede externa pode usar a VMNIC da rede interna em caso de falha de rede, pois as VMNICs da rede interna fazem parte deste grupo de portas e estão configuradas em modo de espera. O oposto ocorre na rede externa. A alternância entre as VMNICs ativas e em espera nos dois grupos de portas é crucial para o failover correto das VMs do ONTAP Select durante interrupções de rede.
