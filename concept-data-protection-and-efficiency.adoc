...
Para obter mais informações sobre boas práticas e outros requisitos, consulte as seções link:concept_ha_config.html#two-node-ha-versus-multi-node-ha["Ha de dois nós versus HA de vários nós"] e link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["Práticas recomendadas de HA (MetroCluster SDS) com dois nós esticados"].
