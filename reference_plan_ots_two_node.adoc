...
Como os clusters de dois nós são normalmente implantados em um escritório remoto ou filial, eles podem estar distantes do data center corporativo e do utilitário ONTAP Select Deploy que fornece suporte administrativo. Com essa configuração, o tráfego de gerenciamento entre o utilitário ONTAP Select Deploy e o cluster flui pela WAN. Consulte o link:reference_release_notes.html["notas de lançamento"] para obter mais informações sobre limitações e restrições.
